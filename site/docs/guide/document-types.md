...
条文の引用は、2026-09-11（JST）に houki-egov-mcp で e-Gov 法令 API から取得して確かめています。
...
| どこで読めるか | e-Gov 法令検索（人事院規則・公正取引委員会規則など） | 裁判所のウェブサイト。e-Gov 法令 API には見当たりません（2026-09-11 に `search_law` で「民事訴訟規則」「刑事訴訟規則」を検索して 0 件） | 各自治体の例規集 |
...
| どこで読めるか | 官報、各省庁のサイト |
| 担当 MCP | なし。2026-09-11 に houki-egov-mcp の `search_law` で「告示」を検索したところ、返ったのは名前に「告示」を含む省令 1 件で、告示そのものは見つかりませんでした |
| 応答での印 | — |
...
| 応答での印 | `legal_status`。基本通達では、解釈の対象になる法律も `base_laws`（検索では `base_laws_by_tsutatsu`）で示します（houki-nta-mcp v0.11.0 から） |

国民を拘束しないといっても、税務署や労働基準監督署は通達に沿って判断します。そのため実務では通達を踏まえて手続を進め、通達の内容を争うときは法律の条文に戻って主張することになります。

houki-nta-mcp は、通達を返すたびに次の `legal_status` を付けます（v0.11.0 の `src/constants.ts` の `TSUTATSU_LEGAL_STATUS`）。
...
裁決と判例は対象外で、`name: "裁決"` を渡すと `found: false` が返ります（v0.5.3、2026-09-11 に確認）。
