...
houki-hub は、**実装する前に、その仕様が法令のどこに触れるかを条文で確かめる**ための部品を束ねたものです。
日本の法令・通達・行政解釈を LLM から引き、「法律で決まっている」と「通達でそうなっている」を混ぜずに、出典と鮮度を添えて返します。
...
- **MCP を組み込む開発者**: 問いを投げるのではなく、`tools/list` の `inputSchema`・エラーコード・`next_actions` の契約を読んで自分のエージェントに組み込みます。契約は houki-research Skill の `docs/` に正典があります
