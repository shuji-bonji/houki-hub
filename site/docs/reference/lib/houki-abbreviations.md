...
description: "houki-abbreviations v0.5.2 の公開 API（関数 19 個・定数 6 個・インターフェース 11 個・型 6 個）のシグネチャ・追加された版・family での使用状況（dist/index.d.ts から自動生成）"
---
...
::: info
**v0.5.2** の `dist/index.d.ts` から自動生成しました（関数 19 個・定数 6 個・インターフェース 11 個・型 6 個・2026-09-20）。手で編集しないでください。再生成は `node scripts/generate-reference.mjs` です。
:::
