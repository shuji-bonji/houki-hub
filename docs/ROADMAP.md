...
graph TB
    ...
    subgraph MCP層["MCP 層（SDK v2 / Node 22 / Biome に統一済み）"]
        EGOV["houki-egov-mcp v0.9.0<br/>e-Gov 法令 API v2 / 7 tools<br/>bulk DL → SQLite FTS5 全文検索まで完了"]
...
| 2026-09-19 | houki-egov-mcp v0.9.0（コードの変更なし。0.8.0 の取り込み後に main の履歴を fixup で整理した際に版だけが上がり、npm に publish 済みだったため、`server.json` を 0.9.0 にして Registry にも登録、CHANGELOG に「変更なし」の項を立てた。ブランチ `chore/0.9.0-registry`）。claude-plugins も 0.9.0 |
