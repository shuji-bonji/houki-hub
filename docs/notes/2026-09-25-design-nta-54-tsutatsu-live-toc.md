...
- 状態: 設計の草案。決まったら仕様 PR（`specs/changes/<日付>-tsutatsu-live-toc/`）に書き直す
...
目次を呼び出しのたびに取得すると、1 回の呼び出しで国税庁サイトへのアクセスが 1 回増えます。目次は DB に保存し、次の呼び出しから使い回します。

- 保存先: 新しい表 `tsutatsu_toc`（通達名、解析結果の JSON、取得日時、`Last-Modified`、`ETag`）
- 使い回す期間: 決めること（6 章の 2）。期間を過ぎたら条件付き取得（Phase 6-2 の差分更新と同じ `If-Modified-Since` / `If-None-Match`）
- 候補ページがどれも 404 のときは、目次が古い可能性があるので、目次を 1 回だけ取り直して候補を作り直す

### 4.4 候補ページの取得

| 項目 | 設計 |
|---|---|
| 1 回の呼び出しで取るページ数の上限 | 決めること（6 章の 3）。上限に達したら `ARTICLE_NOT_FOUND` に `searched_urls` と「bulk download すれば全ページを持てる」案内を付ける |
| 取得の間隔 | ページとページの間に待ちを入れる（bulk download は 1.1 秒）。対話の応答時間との兼ね合いで決める（6 章の 4） |
...
- 既存の DB の移行: bulk download が書いた節には `last_modified` か `etag` が入り、書き戻しの節には入らない。どちらかが入った節を持つ通達は `bulk_completed_at` を埋める（決めること 6 章の 5）
...
| # | 項目 | 案 |
|---|---|---|
| 1 | 案 A を先に入れるか | 入れない（3 章）。案 B が長引くなら段階 1 だけ先に出す |
| 2 | 目次を使い回す期間 | 7 日、または bulk download と同じく期限を設けず、候補がどれも 404 のときだけ取り直す |
| 3 | 1 回の呼び出しで取るページ数の上限 | 5。相続税法基本通達の 1 条あたりのページ数（第 3 条で 3 ページ）と、法人税基本通達の 1 節あたりの款の数（第 2 章第 3 節で 10）を見て決める。10 にすると法人税基本通達の款を取りこぼさない |
| 4 | ページの間の待ち | 0.3 秒程度。bulk download の 1.1 秒は全節を連続で取るとき用 |
| 5 | 既存の DB の `bulk_completed_at` | `last_modified` / `etag` のある節を持つ通達を「済み」とみなす。または移行では埋めず、次の bulk download で埋める（それまでは DB に無い条項は国税庁サイトへ進む） |
| 6 | 段階を分けるか | 段階 1: 4.5（bulk 済みの記録）・4.6（全角）・009 の出し分け。段階 2: 4.2〜4.4（目次で候補を決める）。仕様 PR は 1 本、実装 PR は 1〜2 本 |
